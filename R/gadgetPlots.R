#' Plot Gadget fit
##'
##' Plot the results from gadget.fit. The function produces a different plots by datatype and plottype. 
##' Valid datatypes are:
##' \describe{
##'   \item{sidat}{Surveyindices}
##'   \item{summary}{Likelihood summary data}
##'   \item{catchdist.fleets}{Catchdistribution data}
##'   \item{stockdist}{Stockdistribution data}
##'   \item{res.by.year}{Results by year}
##'   \item{stock.std}{Age composition from the model} 
##'   \item{suitability}{Suitability estimated from the model by year and step} 
##' }
##' and valid plottypes are:
##' \describe{
##'   \item{direct}{Default value, plots direct comparisons of data with model output. Valid for all datatype except 'res.by.year'}
##'   \item{weighted}{Only for 'summary'. Plots the weighted likelihood value for each component.}
##'   \item{pie}{Only for 'summary'. Plots the likelihood composition as a pie chart}
##'   \item{lengths}{Only for 'sidat'. Plot the surveyindex based on the SI length group instead of component name.}
##'   \item{bio}{Only for 'sidat'. Plot the biomass weighted survey index, assumes length based abundance index.}
##'   \item{x-y}{Only for 'sidat'. Produces a x-y scatter-plot for the fitted and observed index.}
##'   \item{resid}{Only for 'catchdist.fleets'. Produces a residual plot for each component.}
##'   \item{bubble}{Only for 'catchdist.fleets'. Produces a bubble plot for each component.}
##'   \item{growth}{Only for 'catchdist.fleets'. Produces a plot of fitted growth for each age-length component.}
##'   \item{resid}{Only for 'catchdist.fleets'. Produces a residual plot for each component.}
##'   \item{F}{Only for 'res.by.year'. Produces a F plot by stock.}
##'   \item{total}{Only for 'res.by.year'. Produces a total biomass plot by stock.}
##'   \item{catch}{Only for 'res.by.year'. Produces a total catch plot by stock.}
##'   \item{rec}{Only for 'res.by.year'. Produces a recruitment biomass plot by stock.}
##'   
##'   
##'   
##' }
##' @title plot gadget fit
##' @name plot.gadget.fit
##' @param fit results from gadget fit
##' @param type The type of plot that is returned, depends on the input data (see details)
##' @param data what results should be plotted
##' @return ggplot2::ggplot object
##' @author Bjarki Þór Elvarsson
##' @export
##' @examples 
##' \dontrun{
##' ## collect the data from gadget
##' fit <- gadget.fit()
##' ## plot survey indices
##' plot(fit) 
##' ## plot likelihood
##' plot(fit,data='summary')
##' }
plot.gadget.fit <- function(fit,data = 'sidat',type='direct'){
  pl <- NULL
  
  if(data=='summary' & type == 'direct'){
    pl <- 
      fit$likelihoodsummary %>% 
      dplyr::filter(year!='all') %>% 
      dplyr::mutate(year = as.numeric(year)) %>%
      ggplot2::ggplot(ggplot2::aes(year, likelihood_value)) +
      ggplot2::geom_point() + 
      ggplot2::facet_wrap(~component,scale='free_y') +
      ggplot2::labs(x='Year',y='Score') 
    
  } 
  
  
  
  if(data == 'summary' & type == 'weighted'){
    pl <-  
      fit$likelihoodsummary %>% 
      dplyr::filter(year!='all') %>% 
      dplyr::mutate(year = as.numeric(year)) %>%
      ggplot2::ggplot(ggplot2::aes(year, weight*likelihood_value)) +
      ggplot2::geom_point() + 
      ggplot2::facet_wrap(~component,scale='free_y') +
      ggplot2::labs(x='Year',y='Weighted score')
    
  } 
  
  if(data == 'summary' & type == 'pie'){
    pl <- 
      fit$likelihoodsummary %>% 
      dplyr::group_by(component) %>% 
      dplyr::summarise(val = sum(likelihood_value*weight)) %>% 
      ggplot2::ggplot(ggplot2::aes(x="",y=val,fill = component)) + 
      ggplot2::geom_bar(stat='identity',width = 1) + 
      ggplot2::coord_polar("y",start = 0) + 
      ggplot2::scale_fill_brewer(palette="Spectral")
    
  } 
  
  if(data == 'sidat' & type == 'lengths'){
    pl <-
      ggplot2::ggplot(fit$sidat, 
                      ggplot2::aes(year,observed)) +
      ggplot2::geom_point() +
      ggplot2::geom_line(ggplot2::aes(year,predict)) +
      ggplot2::geom_linerange(data=subset(fit$sidat,year==max(year)),
                              ggplot2::aes(year,ymax=observed,ymin=predict),col='green')+
      ggplot2::geom_text(data=plyr::ddply(fit$sidat,~length,function(x){
        mutate(subset(x,year==min(year)),y=Inf)
      }),
      ggplot2::aes(year,y,label=length), vjust = 2,hjust = -1)+
      ggplot2::facet_wrap(~lower+step,scale='free_y',ncol=2) + 
      ggplot2::labs(y='Index', x='Year') +
      ggplot2::theme(panel.spacing = ggplot2::unit(0,'cm'), 
                     plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                     strip.background = ggplot2::element_blank(), 
                     strip.text.x = ggplot2::element_blank())
    
  } 
  
  if(data == 'sidat' & type == 'direct'){
    pl <-
      ggplot2::ggplot(fit$sidat, 
                      ggplot2::aes(year,observed)) +
      ggplot2::geom_point() +
      ggplot2::geom_line(ggplot2::aes(year,predict)) +
      ggplot2::geom_linerange(data=fit$sidat %>% 
                                dplyr::filter(year==max(year)),
                              ggplot2::aes(year,ymax=observed,ymin=predict),col='green') +
      ggplot2::geom_text(data=fit$sidat %>% 
                           dplyr::group_by(name) %>% 
                           dplyr::filter(year==min(year)) %>% 
                           dplyr::mutate(y=Inf),
                         ggplot2::aes(year,y,label = name), vjust = 2,hjust = -1)+
      ggplot2::facet_wrap(~name,scale='free_y',ncol=2) + 
      ggplot2::labs(y='Index', x='Year') +
      ggplot2::theme (panel.spacing = ggplot2::unit(0,'cm'), 
                      plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                      strip.background = ggplot2::element_blank(), 
                      strip.text.x = ggplot2::element_blank())
  }
  
  if(data=='sidat' & type == 'bio'){
    bio.tmp <- 
      fit$sidat %>% 
      dplyr::group_by(year,step) %>% 
      dplyr::summarise(obs = sum(observed*bio),
                       prd = sum(predict*bio)) 
    pl <-
      ggplot2::ggplot(bio.tmp, ggplot2::aes(year,obs)) +
      ggplot2::geom_point() +
      ggplot2::geom_line(ggplot2::aes(year,prd)) +
      ggplot2::geom_linerange(data=bio.tmp %>% 
                                dplyr::filter(year==max(year)),
                              ggplot2::aes(year,ymax=obs,ymin=prd),col='green')+
      ggplot2::facet_wrap(~step,scale='free_y',ncol=2)  +
      ggplot2::labs(y='Biomass index', x='Year') +
      ggplot2::theme (panel.spacing = ggplot2::unit(0,'cm'), 
                      plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                      strip.background = ggplot2::element_blank(), 
                      strip.text.x = ggplot2::element_blank())
  } 
  
  if(data == 'sidat' & type == 'x-y' ) {
    pl <-
      ggplot2::ggplot(fit$sidat,ggplot2::aes(observed,predict,label=year)) + 
      ggplot2::geom_text() + 
      ggplot2::geom_abline(slope = 1, lty = 2) +
      ggplot2::facet_wrap(~name,scale='free') + 
      ggplot2::labs(y='Predicted value', x='Observed') + 
      ggplot2::geom_hline(data=filter(fit$sidat,year==max(year)),
                          ggplot2::aes(yintercept=predict),col='green') + 
      ggplot2::geom_vline(data=dplyr::filter(fit$sidat,year==max(year)),
                          ggplot2::aes(xintercept=predict),col='green') + 
      ggplot2::geom_text(data = fit$sidat %>% 
                           dplyr::select(name) %>% 
                           dplyr::distinct(),
                         ggplot2::aes(-Inf,Inf,label=name),vjust=2,hjust=-0.5) + 
      ggplot2::theme(panel.spacing = ggplot2::unit(0,'cm'), 
                     plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                     strip.background = ggplot2::element_blank(), 
                     strip.text.x = ggplot2::element_blank())
    
  } 
  
  if(data=='catchdist.fleets'){
    if(type == 'direct'){
      pl <-  
        unique(fit$catchdist.fleets$name) %>% 
        purrr::set_names(.,.) %>% 
        purrr::map(function(x){
          dat <- fit$catchdist.fleets %>% 
            dplyr::filter(name == x)
          if(length(unique(dat$age))==1){
            dat %>% 
              dplyr::ungroup() %>% 
              ggplot2::ggplot(ggplot2::aes(lower,predicted)) +
              ggplot2::geom_line(ggplot2::aes(lower,observed),col='gray') +
              ggplot2::facet_wrap(~year+step,drop = FALSE,
                                  ncol = max(2*length(unique(dat$step)),4))  + 
              ggplot2::geom_line() +
              ggplot2::geom_text(data=dat %>% 
                                   dplyr::ungroup() %>% 
                                   dplyr::filter(lower == min(lower)) %>% 
                                   dplyr::mutate(y=Inf,
                                                 label = paste(year,step,sep=',')) %>% 
                                   dplyr::select(step,lower,y,year,label), 
                                 
                                 ggplot2::aes(lower,y,label=label), 
                                 vjust = 1.3,hjust = -.05,
                                 size = 3,
                                 inherit.aes = FALSE)+
              ggplot2::labs('Proportion', x='length') +
              ggplot2::theme (axis.text.y = ggplot2::element_blank(),
                              axis.ticks.y = ggplot2::element_blank(),
                              panel.spacing = ggplot2::unit(0,'cm'),
                              plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                              strip.background = ggplot2::element_blank(),
                              strip.text.x = ggplot2::element_blank())
          } else {
            dat %>% 
              dplyr::group_by(year,step,age) %>% 
              dplyr::summarise(predicted=sum(predicted),
                               observed=sum(observed,na.rm=TRUE)) %>% 
              dplyr::mutate(age = as.numeric(gsub('age','',age))) %>% 
              dplyr::ungroup() %>% 
              ggplot2::ggplot(ggplot2::aes(age,predicted)) +
              ggplot2::geom_line(ggplot2::aes(age,observed),col='gray') +
              ggplot2::facet_wrap(~year+step,drop = FALSE,
                                  ncol = max(2*length(unique(dat$step)),4))  + 
              ggplot2::geom_line() +
              ggplot2::geom_text(data=dat %>% 
                                   dplyr::ungroup() %>% 
                                   dplyr::mutate(age = as.numeric(gsub('age','',age))) %>% 
                                   dplyr::filter(age == min(age)) %>% 
                                   dplyr::mutate(y=Inf,
                                                 label = paste(year,step,sep=',')) %>% 
                                   dplyr::select(step,age,y,year,label) %>% 
                                   dplyr::distinct(), 
                                 ggplot2::aes(age,y,label=label), 
                                 vjust = 1.3,hjust = -.05,
                                 size = 3,
                                 inherit.aes = FALSE)+
              ggplot2::labs(y='Proportion', x='Age') +
              ggplot2::theme (axis.text.y = ggplot2::element_blank(),
                              axis.ticks.y = ggplot2::element_blank(),
                              panel.spacing = ggplot2::unit(0,'cm'),
                              plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                              strip.background = ggplot2::element_blank(),
                              strip.text.x = ggplot2::element_blank())
          }
        })
      
    } else if(type == 'growth'){
      fit$catchdist.fleets %>%
        dplyr::ungroup() %>% 
        tidyr::nest(-name) %>% 
        dplyr::mutate(plots = purrr::map(data,function(x){
          if(length(unique(x$age))>1)
            x %>% 
            dplyr::group_by(year,step,age) %>% 
            dplyr::mutate(o=observed/sum(observed,na.rm=TRUE),p=predicted/max(sum(predicted),1e-14)) %>% 
            dplyr::select(year,step,age,length = avg.length,observed,o,predicted,p) %>% 
            dplyr::ungroup() %>% 
            dplyr::mutate(age=as.numeric(gsub('age','',age))) %>% 
            dplyr::group_by(year,step,age) %>% 
            dplyr::summarise(o.ml=sum(o*length,na.rm=TRUE),
                             o.sl=sqrt(sum(o*(length - o.ml)^2,na.rm=TRUE)),
                             p.ml=sum(p*length),
                             p.sl=sqrt(sum(p*(length - p.ml)^2))) %>% 
            dplyr::mutate(o.ml=ifelse(o.ml==0,NA,o.ml),
                          o.sl=ifelse(o.sl==0,NA,o.sl),
                          upper = p.ml+1.96*p.sl,
                          lower = p.ml-1.96*p.sl,
                          o.upper = o.ml+1.96*o.sl,
                          o.lower = o.ml-1.96*o.sl) %>% 
            ggplot2::ggplot(ggplot2::aes(age,o.ml)) + 
            ggplot2::geom_ribbon(fill='gold',ggplot2::aes(ymax=upper,ymin=lower))+
            ggplot2::geom_point(size=0.5) + 
            ggplot2::geom_line(ggplot2::aes(y=p.ml))  + 
            ggplot2::geom_linerange(ggplot2::aes(ymax=o.upper,ymin=o.lower))+
            ggplot2::facet_wrap(~year+step,drop = FALSE,
                                ncol = max(2*length(unique(x$step)),4)) + 
            ggplot2::labs(x='Age', y='Average length') +
            ggplot2::geom_text(x=-Inf,y=Inf,
                               ggplot2::aes(label=paste(year,step,sep=',')),
                               size=3,
                               data = x %>% 
                                 dplyr::select(year,step) %>% 
                                 dplyr::distinct(),vjust = 1.5,hjust = -0.1,
                               inherit.aes = FALSE) + 
            ggplot2::theme(strip.background = ggplot2::element_blank(),strip.text=ggplot2::element_blank()) 
        })) %>% 
        dplyr::filter(map(plots,~!is.null(.)) %>% unlist()) %>% 
        dplyr::select(name,plots) -> tmp
      
      pl <-
        tmp$plots %>% 
        purrr::set_names(.,tmp$name)
      
    } else if(type == 'resid'){
      pl <-
        ggplot2::ggplot(fit$catchdist.fleets,
                        ggplot2::aes(lower, observed - predicted,
                                     group=round(lower,1))) +
        ggplot2::geom_boxplot() + 
        ggplot2::facet_wrap(~name)  + 
        ggplot2::labs(y='Residual', y='Length')
    } else if(type == 'bubble'){
      
      pl <- 
        list(ldist = 
               fit$catchdist.fleets %>% 
               dplyr::group_by(name) %>% 
               dplyr::mutate(n=dplyr::n_distinct(age)) %>% 
               dplyr::filter(n==1,
                             abs(observed-predicted)!=0) %>% 
               dplyr::left_join(fit$SS$weights %>% rename(name=Component)) %>% 
               ggplot2::ggplot(ggplot2::aes(year+(step-1)/4,avg.length,size=abs((observed-predicted)*sqrt(Weight)), 
                                            col=as.factor(sign((observed-predicted))))) + 
               ggplot2::geom_point() + 
               ggplot2::facet_wrap(~name)  + 
               ggplot2::scale_color_manual(values=c('darkblue','red')) + 
               ggplot2::scale_size_area() + 
               ggplot2::theme(legend.position = 'none') + 
               ggplot2::labs(x='Year',y='Length'),
             aldist = 
               fit$catchdist.fleets %>% 
               dplyr::group_by(name) %>% 
               dplyr::mutate(n=dplyr::n_distinct(age)) %>% 
               dplyr::filter(n!=1) %>% 
               dplyr::mutate(age=as.numeric(gsub('age','',age))) %>% 
               dplyr::group_by(name,year,age,step,total.catch) %>% 
               dplyr::summarise(o=sum(observed,na.rm=TRUE),
                                p=sum(predicted)) %>% 
               dplyr::mutate(o=ifelse(o==0,NA,o)) %>%
               dplyr::filter(abs(o-p)!=0) %>% 
               dplyr::left_join(fit$SS$weights %>% rename(name=Component)) %>% 
               ggplot2::ggplot(ggplot2::aes(year+(step-1)/4,age,size=abs((o-p)*sqrt(Weight)), 
                                            col=as.factor(sign((o-p))))) + ggplot2::geom_point() + 
               ggplot2::facet_wrap(~name)  + 
               ggplot2::scale_color_manual(values=c('darkblue','red'))+ 
               ggplot2::theme(legend.position = 'none'))
    }
  }
  
  if(data == 'res.by.year' & type == 'F'){
    pl <-
      ggplot2::ggplot(fit$res.by.year %>% ungroup(),ggplot2::aes(year,F,col=stock)) + 
      ggplot2::geom_line() +
      ggplot2::labs(y='F', x='Year')
    
  } 
  
  if(data == 'res.by.year' & type == 'rec'){
    pl <-
      ggplot2::ggplot(fit$res.by.year,ggplot2::aes(year,recruitment/1e6,col=stock)) + ggplot2::geom_line() +
      ggplot2::labs(y='Recruitment (in millions)', x='Year')
    
  } 
  
  if(data == 'res.by.year' & type == 'total'){
    pl <-
      ggplot2::ggplot(fit$res.by.year,ggplot2::aes(year,total.biomass/1e6,col=stock)) + 
      ggplot2::geom_line() +
      ggplot2::labs(y="Biomass (in '000 tons)", x='Year')
    
  } 
  
  if(data == 'res.by.year' & type == 'num.total'){
    pl <-
      ggplot2::ggplot(fit$res.by.year,ggplot2::aes(year,total.number/1e6,col=stock)) + 
      ggplot2::geom_line() +
      ggplot2::labs(y="Abundance (in millions)", x='Year')
    
  } 
  
  if(data == 'res.by.year' & type == 'ssb'){
    pl <-
      ggplot2::ggplot(fit$res.by.year,ggplot2::aes(year,ssb/1e6,col=stock)) + 
      ggplot2::geom_line() +
      ggplot2::labs(y = "SSB (in '000 tons)", x = 'Year')
    
  } 
  
  if(data == 'res.by.year' & type == 'catch'){
    pl <-
      ggplot2::ggplot(fit$res.by.year,ggplot2::aes(year,catch/1e6,fill=stock)) + 
      ggplot2::geom_bar(stat='identity') +
      ggplot2::labs(y="Catch (in '000 tons)",x='Year')
    
  } 
  
  if(data == 'res.by.year' & type == 'num.catch'){
    pl <-
      ggplot2::ggplot(fit$res.by.year,ggplot2::aes(year,num.catch/1e6,fill=stock)) + 
      ggplot2::geom_bar(stat='identity') +
      ggplot2::labs(y="Catch in numbers (millions)",x='Year')
    
  } 
  
  if(data == 'suitability') {
    pl <-
      fit$suitability %>%
      dplyr::ungroup() %>% 
      ggplot2::ggplot(ggplot2::aes(length,suit,lty=fleet)) +
      ggplot2::geom_line() +
      ggplot2::facet_wrap(~year + step) +
      ggplot2::labs(y='Suitability',x='Length') +
      ggplot2::geom_text(data=fit$suitability %>%
                           dplyr::ungroup() %>% 
                           dplyr::select(year,step) %>% 
                           dplyr::mutate(y=Inf,
                                         label = paste(year,step,sep=',')) %>% 
                           dplyr::select(step,y,year,label) %>% 
                           dplyr::distinct(), 
                         ggplot2::aes(-Inf,Inf,label=label), 
                         vjust = 1.3,hjust = -.05,
                         size = 3,
                         inherit.aes = FALSE) +
      ggplot2::facet_wrap(~year+step,drop = FALSE,
                          ncol = max(2*length(unique(fit$suitability$step)),4))  + 
      ggplot2::theme(axis.text.y = ggplot2::element_blank(),
                     axis.ticks.y = ggplot2::element_blank(),
                     panel.spacing = ggplot2::unit(0,'cm'),
                     plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                     strip.background = ggplot2::element_blank(),
                     strip.text.x = ggplot2::element_blank(), 
                     legend.title = ggplot2::element_blank())
    
  } 
  
  if(data == 'growth') {
    pl <-
      fit$stock.std %>% 
      dplyr::filter(step == 1,number > 0) %>% 
      ggplot2::ggplot(ggplot2::aes(age,mean.length,lty=stock)) + 
      ggplot2::geom_line() +
      ggplot2::facet_wrap(~year) + 
      ggplot2::labs(y='Length',x='Age') +
      ggplot2::theme(legend.title = ggplot2::element_blank(),
                     plot.margin = ggplot2::unit(c(0,0,0,0),'cm'))
  } 
  
  if(data == 'stock.std') {
    year_span <- 
      unique(fit$stock.std$year)
    
    pl <-
      fit$stock.std %>% 
      dplyr::mutate(yc = as.factor(year - age)) %>% 
      ggplot2::ggplot(ggplot2::aes(year,number,fill=yc),col='black') + 
      ggplot2::geom_bar(stat='identity') + 
      ggplot2::facet_wrap(~age,ncol=1,scale='free_y') + 
      ggplot2::theme(legend.position='none',panel.spacing = ggplot2::unit(0,'cm'),
                     plot.margin = ggplot2::unit(c(0,0,0,0),'cm'),
                     strip.background = ggplot2::element_blank(),
                     strip.text.x = ggplot2::element_blank()) + 
      ggplot2::annotate("segment", 
                        x=year_span-0.5, 
                        xend=year_span+.5,
                        y=Inf, yend=-Inf,lty=2,col='gray') + 
      ggplot2::geom_text(ggplot2::aes(Inf,Inf,label=paste('Age',age)),
                         hjust=2,vjust=2,col='gray')+
      ggplot2::labs(x='Year',y='Num. fish (in millions)') 
  }
  
  if(data == 'stockdist'){
    
    fit$stockdist %>%
      tidyr::nest(-name) %>% 
      dplyr::mutate(plots = purrr::map(data,function(x){
        if(x$length %>% unique() %>% length() > 1){
          x %>% 
            dplyr::mutate(pred.ratio= ifelse(is.nan(pred.ratio),0,pred.ratio)) %>% 
            ggplot2::ggplot(ggplot2::aes(length,obs.ratio)) +
            ggplot2::geom_point() + 
            ggplot2::geom_line(ggplot2::aes(y=pred.ratio,lty = stock))+
            ggplot2::facet_wrap(~year+step) + ggplot2::theme_light() + 
            ggplot2::labs(y='Stock prop.',x='Length') +
            ggplot2::geom_label(data= fit$stockdist %>% 
                                  dplyr::ungroup() %>% 
                                  dplyr::select(year,step) %>% 
                                  dplyr::distinct()%>% 
                                  dplyr::mutate(label=paste(year,step, sep=',')),
                                ggplot2::aes(label=label,group=1),
                                x=-Inf,y=Inf,size=3, vjust = 2,hjust=-0.1) +
            ggplot2::theme(strip.background = ggplot2::element_blank(),
                           strip.text = ggplot2::element_blank())
        } else {
          x %>% 
            dplyr::mutate(pred.ratio= ifelse(is.nan(pred.ratio),0,pred.ratio),
                          age = gsub('age','',age) %>% as.numeric()) %>% 
            ggplot2::ggplot(ggplot2::aes(age,obs.ratio,col=stock)) +
            ggplot2::geom_point() + 
            ggplot2::geom_line(ggplot2::aes(y=pred.ratio,lty = stock))+
            ggplot2::facet_wrap(~year+step) + 
            ggplot2::theme_light() + 
            ggplot2::labs(y='Stock prop.',x='Age') +
            ggplot2::geom_label(data= fit$stockdist %>% 
                                  dplyr::ungroup() %>% 
                                  dplyr::select(year,step) %>% 
                                  dplyr::distinct()%>% 
                                  dplyr::mutate(label=paste(year,step, sep=',')),
                                ggplot2::aes(label=label,group=1),
                                x=-Inf,y=Inf,size=3, vjust = 2,hjust=-0.1,
                                inherit.aes = FALSE) +
            ggplot2::theme(strip.background = ggplot2::element_blank(),
                           strip.text = ggplot2::element_blank())
        }
      })) %>% 
      dplyr::filter(purrr::map(plots,~!is.null(.)) %>% unlist()) %>% 
      dplyr::select(name,plots) -> tmp
    
    pl <-
      tmp$plots %>% 
      purrr::set_names(.,tmp$name)
    
  }
  
  return(pl)
}

